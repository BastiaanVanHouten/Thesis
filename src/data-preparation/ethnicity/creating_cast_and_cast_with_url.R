...
cast_2018_2019 <- read_csv("../../../data/cast_ethnicity/movie_cast_2018_2019.csv")
...
write.csv(cast, file = "gen/data-preparation/temp/combination_cast_main_base_2018_2019_not_done.csv", row.names = FALSE)


# filtering for cast without url and in Master
filtered_cast <- cast %>%
    filter(
        `Movie ID` %in% master_cleaned$imdb.com_imdbid
    )
